...
def ground_truth_table(config: ExperimentConfig, reward_family: str, agent_id: int) -> np.ndarray:
    ...
    table = np.zeros(
        (config.num_states, config.num_actions, config.num_actions), dtype=np.float32
    )
...
def run_single_experiment(
    ...
    predicted_tables = [
        table.detach().cpu().numpy() for table in malfl_artifacts.reward_learning.predicted_reward_tables
    ]

    evaluation: RewardEvaluationResult | None = malfl_artifacts.evaluation
    if evaluation is not None:
        for agent_idx, metrics in enumerate(evaluation.agent_metrics):
            logging.info(
                "[Experiment:%s] Agent %d metrics - Pearson: %.4f | Spearman: %.4f",
                reward_family,
                agent_idx,
                metrics.pearson,
                metrics.spearman,
        ...
        if evaluation.trend_stages:
            logging.info(
                ...
                reward_family,
                ", ".join(str(stage) for stage in evaluation.trend_stages),
            )
            logging.info(
