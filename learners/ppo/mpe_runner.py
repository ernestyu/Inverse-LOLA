"""Simple PPO training loop on PettingZoo MPE simple_spread."""
...
from envs.mpe_simple_spread import make_env
...
class SharedPolicy(nn.Module):
    ...
    def evaluate_actions(self, obs: torch.Tensor, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        std = self.log_std.exp()
        dist = torch.distributions.Normal(mean, std)
        clipped_actions = torch.clamp(actions, 0.0, 1.0)
...
def collect_rollout(env, policy: SharedPolicy, steps: int, device: torch.device, gamma: float, gae_lambda: float):
    ...
    for _ in range(steps):
        actions = {}
        # compute actions for alive agents
        for aid in list(active_agents):
            obs_np = np.asarray(env_obs[aid], dtype=np.float32)
            obs_tensor = torch.as_tensor(obs_np, device=device)
            with torch.no_grad():
                act, logp, value = policy.act(obs_tensor)
        ...
        # record rewards/dones for active agents
        finished_this_step = []
        for aid in list(active_agents):
    ...
    # compute advantages/returns per agent
    ...
    # flatten across agents for PPO update
    ...
    return PPORollout(agent_rollouts=rollouts), obs_tensor, acts_tensor, adv_tensor, ret_tensor, old_logp_tensor
...
def ppo_update(
    ...
    for _ in range(epochs):
        ...
        for start in range(0, obs.size(0), minibatch_size):
            ...
            new_logp, entropy, values = policy.evaluate_actions(mb_obs, mb_actions)
...
def run_ppo_training(config: dict, seed: int = 0, save_dir: Path | None = None):
    ...
    max_cycles = int(config.get("max_cycles", 50))
    steps_per_rollout = int(config.get("steps_per_rollout", 64))
    updates = int(config.get("updates", 5))
    ...
    raw_checkpoints = []
    save_dir = save_dir or Path("outputs") / "raw" / "t2"
    save_dir.mkdir(parents=True, exist_ok=True)

    for update_idx in range(updates):
        rollout, obs_tensor, act_tensor, adv_tensor, ret_tensor, old_logp_tensor = collect_rollout(
        ...
        # persist raw rollout + theta for traceability
        out_path = save_dir / f"ppo_mpe_update{update_idx}.pkl"
        with out_path.open("wb") as f:
            import pickle

            pickle.dump(ckpt, f)
        print(f"[ppo] update {update_idx}: saved {out_path}, stats={stats}")
