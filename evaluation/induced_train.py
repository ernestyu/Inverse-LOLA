        ...
        while not done:
            if policy is None:
                ...
                for aid in env.agents:
                    ...
                    with torch.no_grad():
                        # MultiWalker policy requires bounds; fall back to per-agent bounds if not provided.
                        ...
                        low = low.to(dev)
                        high = high.to(dev)
                        act, _, _ = policy.act(obs_t, low, high)
...
def run_induced_training_t2(
    ...
    R_random = eval_policy(env, None, episodes=int(cfg.get("eval_episodes", 5)))
    # expert baseline (optional)
    R_expert = np.nan
    if expert_ckpt_path and Path(expert_ckpt_path).exists():
        R_expert = eval_policy(env, policy=None, episodes=int(cfg.get("eval_episodes", 5)))  # placeholder; no loading
    ...
    for update_idx in range(num_updates):
        ...
        if (update_idx + 1) % eval_every == 0:
            R_ind = eval_policy(env, policy, episodes=eval_episodes)
...
def run_induced_training_t3(
    ...
    R_random = eval_policy(env, None, episodes=int(cfg.get("eval_episodes", 5)), action_low=action_low, action_high=action_high)
    R_expert = np.nan
    ...
    for update_idx in range(num_updates):
        ...
        reinforce_update(policy, optimizer, storage)
        if (update_idx + 1) % eval_every == 0:
            R_ind = eval_policy(env, policy, episodes=eval_episodes, action_low=action_low, action_high=action_high)
            R_induced_curve.append((update_idx + 1, R_ind))
